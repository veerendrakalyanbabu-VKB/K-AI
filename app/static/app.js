...
function initGlobe() {
 ...
 globe.htmlLat('lat').htmlLng('lng').htmlAltitude(d=>d.layer==='space'?(d.altitude||.015):.002).htmlElement(markerElement);
...
function showDetail(item){
 ...
 const details={...item.metrics,registration:item.registration,icao24:item.icao24,callsign:item.callsign,aircraft_type:item.aircraft_type,squawk:item.squawk,vertical_rate_fpm:item.vertical_rate_fpm,on_ground:item.on_ground,norad_id:item.norad_id,international_id:item.international_id,inclination_degrees:item.inclination_degrees,destination:item.destination,reported_eta:item.reported_eta,imo:item.imo,callsign:item.callsign,ship_type:item.ship_type,nav_status:item.nav_status,dimensions:item.dimensions,voyage_reported_at:item.voyage_reported_at,altitude_km:item.altitude_km,altitude_m:item.altitude_m,altitude_ft:item.altitude_ft,speed_kn:item.speed_kn,speed_ms:item.speed_ms,heading_degrees:item.heading,element_epoch:item.epoch,received_at:item.received_at};
 ...
 if(['aviation','marine','space'].includes(item.layer)){const button=document.createElement('button');button.textContent='Follow incoming position updates';button.onclick=()=>{following={layer:item.layer,id:item.id};lastFollowPosition='';if(globe)globe.controls().autoRotate=false;$('#rotate').textContent='▶ Rotate';$('#rotate').setAttribute('aria-pressed','false');$('#unfollow').hidden=false;$('#drawer').close();render();};$('#detail').appendChild(button);}
